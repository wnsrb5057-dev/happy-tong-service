...
export async function getSupabaseCheckerTargets(checkerId) {
  ...
  try {
    ...
    return {
      ...
      source: "error",
      targets: [],
      message: error?.message || "Supabase 체커 대상자 목록을 불러오지 못했습니다.",
    };
  }
}

export async function getSupabaseCheckerTargetById(checkerId, targetId) {
  const result = await getSupabaseCheckerTargets(checkerId);
