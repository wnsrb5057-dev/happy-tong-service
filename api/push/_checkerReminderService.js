...
function getVapidConfig() {
  ...
  if (!publicKey || !privateKey || !subject) {
    throw createCodeError("MISSING_VAPID_ENV");
  }
...
async function getActiveCheckerUsers(supabase) {
  ...
  if (error) {
    console.error("[push/checker-reminder] USER_QUERY_FAILED", {
      code: error.code || null,
      message: error.message || "Unknown Supabase error",
    });
    throw createCodeError("USER_QUERY_FAILED");
  }
...
async function getExistingReminderLogs(supabase, targetDate, userIds) {
  ...
  if (error) {
    console.error("[push/checker-reminder] LOG_QUERY_FAILED", {
      code: error.code || null,
      message: error.message || "Unknown Supabase error",
    });
    throw createCodeError("LOG_QUERY_FAILED");
  }
...
async function getActiveCheckerSubscriptions(supabase, userIds) {
  ...
  if (error) {
    console.error("[push/checker-reminder] SUBSCRIPTION_QUERY_FAILED", {
      code: error.code || null,
      message: error.message || "Unknown Supabase error",
    });
    throw createCodeError("SUBSCRIPTION_QUERY_FAILED");
  }
...
async function upsertReminderLog(supabase, existingLogId, payload) {
  if (existingLogId) {
    return supabase.from("push_notification_logs").update(payload).eq("id", existingLogId);
  }

  return supabase.from("push_notification_logs").insert(payload);
}
...
async function markSubscriptionInactive(supabase, subscriptionId) {
  ...
  if (error) {
    console.error("[push/checker-reminder] MARK_INACTIVE_FAILED", {
      subscriptionId,
      code: error.code || null,
      message: error.message || "Unknown Supabase error",
    });
...
export async function runCheckerReminderJob({ date, dryRun = false, source = null } = {}) {
  const targetDate = normalizeTargetDate(date);
  if (!targetDate) {
    throw createCodeError("INVALID_JSON");
  }
  ...
  for (const user of scheduledUsers) {
    ...
    for (const subscription of userSubscriptions) {
      try {
        ...
        console.error("[push/checker-reminder] WEB_PUSH_SEND_FAILED", {
    ...
    if (delivered) {
      ...
      if (error) {
        ...
        console.error("[push/checker-reminder] LOG_WRITE_FAILED", {
          stage: "sent",
          userId: user.id,
          code: error.code || null,
          message: error.message || "Unknown Supabase error",
    ...
    const logPayload = createLogPayload({
      ...
      errorMessage: "Failed to send checker reminder.",
      subscriptionId: representativeSubscriptionId,
    });
