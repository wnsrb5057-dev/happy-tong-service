-- 해피통서비스 Supabase 체커 기록작성 대상자 목록 RPC
-- docs/supabase-schema.sql, docs/supabase-seed.sql 실행 후 사용
-- 체커 기록작성 화면 대상자 선택 목록 읽기 전용 테스트용
-- 실제 운영 전에는 Supabase Auth/RLS로 본인 담당 대상자만 조회하도록 제한 필요
-- 현재 MVP 단계에서는 p_checker_id를 명시적으로 받아 해당 체커의 대상자만 반환
-- 연락처와 상세 메모는 반환하지 않고 대상자 선택에 필요한 요약만 반환

create or replace function public.get_public_checker_activity_form_targets(p_checker_id uuid)
returns table (
  id uuid,
...
select
  ...
  t.age,
  t.gender,
  coalesce(t.address, to_jsonb(t)->>'address', '-') as address,
  coalesce(t.risk_level, to_jsonb(t)->>'risk_level', 'normal') as risk_level,
  coalesce(t.lifecycle_status, to_jsonb(t)->>'lifecycle_status', 'active') as lifecycle_status,
  coalesce(t.default_check_type, to_jsonb(t)->>'default_check_type', 'phone') as default_check_type,
  ...
  coalesce(
    ...
    nullif(to_jsonb(latest_activity)->>'result', ''),
    nullif(to_jsonb(latest_activity)->>'condition_status', ''),
    ''
...
where coalesce(t.lifecycle_status, to_jsonb(t)->>'lifecycle_status', 'active') <> 'ended'
order by
...
grant execute on function public.get_public_checker_activity_form_targets(uuid) to anon;
grant execute on function public.get_public_checker_activity_form_targets(uuid) to authenticated;
