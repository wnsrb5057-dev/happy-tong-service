# 높은 위험 RPC EXECUTE 제거 준비 확인

## 1. 작업 목적

높은 위험 `get_public_*` RPC 6개에 대해 `PUBLIC` / `anon` EXECUTE 제거 전 최종 준비 상태를 확인한다.
...
- 해당 RPC 문자열은 `api/super.js` 내부 service_role 기반 호출에만 남아 있다.
...
- 위 6개 service에서 `get_public_` 검색 결과 없음
- 각 service는 `/api/super` fetch 호출로 전환되어 있다.

## 5. `api/super` 내부 RPC 호출 확인

`api/super.js`에는 아래 RPC 호출이 남아 있다. 이는 service_role 기반 서버 API 내부 호출이므로 정상이다.
...
- 오류 로그는 `[super-api]`, code, message 수준의 `console.warn`만 사용

## 6. `superAdminPages` 화면 흐름 확인

`src/pages/superAdminPages.jsx`는 직접 `supabase.rpc`를 호출하지 않는다.

확인된 호출 흐름:

- `getSupabaseOrganizationSummaries()`
- `getSupabaseRecentEmergencySummaries()`
- `getSupabaseSuperDashboardKpis()`
- `getSupabaseOrganizationDetail(organizationId)`
- `getSupabaseSuperStatusSummaries()`
- `getSupabaseConnectionStatus()`

위 함수들은 각각 service 파일을 통해 `/api/super`를 호출한다.

화면 fallback:

- 기관 목록 fallback: local organization summary
- 최근 이상징후 fallback: local emergency summary
- KPI fallback: local KPI
- 기관 상세 fallback: local organization detail
- 기관 상태 fallback: local status summary

## 7. 권한 제거 가능성 판단

현재 기준으로 높은 위험 RPC 6개의 `PUBLIC` / `anon` EXECUTE 제거를 검토할 수 있는 상태로 판단된다.

판단 근거:

- `src`에서 해당 6개 RPC 직접 호출 없음
- 지정 6개 service의 직접 `.rpc(` 호출 제거 확인
- `api/super.js`에서만 service_role으로 RPC 호출
- `superAdminPages.jsx`는 service 함수만 호출
- 총관리자 화면 fallback 유지
- 배포 후 `/api/super` smoke test 성공 기록 있음

주의:

- 이번 문서는 준비 상태 확인이며 실제 권한 제거를 수행한 것은 아니다.
- 권한 제거 후에는 `/api/super` smoke test와 총관리자 화면 QA를 반드시 수행해야 한다.
...
```sql
grant execute on function public.get_public_health_counts() to public, anon;
grant execute on function public.get_public_organization_summaries() to public, anon;
grant execute on function public.get_public_organization_detail(uuid) to public, anon;
grant execute on function public.get_public_recent_emergency_summaries() to public, anon;
grant execute on function public.get_public_super_dashboard_kpis() to public, anon;
grant execute on function public.get_public_super_status_summaries() to public, anon;
```
...
2. 실행 직후 확인 SQL 수행
3. `/api/super` smoke test 재수행
4. `super_admin / 1234` 로그인 후 총관리자 화면 QA 수행
5. 문제 발생 시 롤백 SQL을 임시 적용하고 원인 확인
6. 이후 관리자/체커 조직 단위 RPC 서버 API 전환 계획으로 이동
